...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/var/log/pphook.log"), logging.StreamHandler()],
)
...
def notify_error(
    ...
    try:
        ...
        template_vars = {
            ...
            "ip_ipam": PHPIPAM_URL.replace("/api", "")
            .replace("http://", "")
            .replace("https://", ""),
        ...
        if not use_generic_email:
            template_vars.update({"username": username, "edit_date": edit_date, "action": action})
        else:
...
def process_address(phpipam, powerdns, address, users, zones):
    ...
    user_email, username, use_generic_email = phpipam.get_user_email_from_changelog(
        changelog, users, GENERIC_EMAIL
    )
    if not user_email:
    ...
    try:
        ...
        success, corrected = powerdns.handle_dns_case(
            status, hostname, ip, details, dns_error_callback
        )
...
def main():
    ...
    logger.info(
        f"Données récupérées: {len(recent_addresses)} récentes, {len(all_addresses)} totales, {len(users)} utilisateurs, {len(zones)} zones DNS"
    )
    ...
    mac_cleaned, mac_processed_items, updated_addresses = phpipam.clean_mac_duplicates(
        all_addresses
    )
        ...
        try:
            ...
            if user_email:
                ...
                if not success and GENERIC_EMAIL:
                    ...
                    logger.warning(
                        f"Échec notification utilisateur pour MAC {duplicate_mac}, retry avec email générique"
                    )
                    notify_error(
                        address=address,
                        hostname=address.get("hostname", "Non défini"),
                ...
                logger.warning(
                    f"Impossible de notifier pour MAC dupliquée {duplicate_info['mac']} - pas d'email"
                )
    ...
    hostname_cleaned, hostname_processed_items, updated_addresses = (
        phpipam.clean_hostname_duplicates(all_addresses, powerdns, zones)
    )
        ...
        try:
            ...
            if user_email:
                ...
                if not success and GENERIC_EMAIL:
                    # Retry avec email générique si échec
                    logger.warning(
                        f"Échec notification utilisateur pour hostname {hostname}, retry avec email générique"
                    )
                    notify_error(
                        address=address,
                        hostname=hostname,
                ...
                logger.warning(
                    f"Impossible de notifier pour hostname dupliqué {duplicate_info['hostname']} - pas d'email"
                )
